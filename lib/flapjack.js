...
(function($, undefined) {
  ...
  $.widget( "ui.flapjack", {
    ...
    // ### `#destroy`
    // Destroy the widget and clean up any created DOM elements.
    destroy: function() {
    ...
    // Initialize a Flapjack on a list.
    _create: function() {
      ...
      menu.left = function (event) {
        ...
        if (newItem && newItem.length) {
          ...
          // latter, and we're at the top, hide the menu and return focus
          // to the text input.
          flapjack.hideMenu();
          flapjack.$input.focus();
        }
      };

      $input
        .addClass( "ui-widget ui-widget-content ui-corner-left ui-autocomplete-input flapjack-input" )
        .bind('keyup.flapjack', function(event) {
          if (event && event.which === $.ui.keyCode.DOWN) {
            flapjack.showMenu();
          }
        });
