(function($, exports) {
  ...
  // Renders a `Flapjack` or `Flapjack.Group` as an `<ol>`. Recurses when
  // a child is a `Flapjack.Group`. Used by `renderToInputAndLists`.
  function renderGroupToList(group, isGroupActive) {
    var $ol = $('<ol class="flapjack"></ol>');
    if (isGroupActive) { $ol.addClass('fj-active'); }
    _(group).each(function(child) {
      var $li = $('<li></li>');
      var $a = $('<a>' + child.text + '</a>');
      if (child instanceof Array) {
        $a
          .append(' ►')
          .data('flapjack-group', child);
        $li
          .append($a)
          .append(renderGroupToList(child, false));
      } else {
        $a
          .data('flapjack-leaf', child)
          .appendTo($li);
      }
      $ol.append($li);
    });
  ...
  // Render a `Flapjack` object as a serach `<input>` and a tree of `<ol>`s
  // and `<li>`s. To use a different renderer, set the `render` property
  // of the Flapjack instance. Called in the context of the `Flapjack`
  // instance.
  function renderToInputAndLists() {
    this.$select.hide();
    this.$select.parent()
      .append('<input type="search" class="flapjack" />')
      .append(renderGroupToList(this, true).hide());
  }
  ...
  Flapjack = function(selector, options) {
  ...
  Flapjack.prototype = $.extend([], {
    ...
    // Render this `Flapjack` to a DOM representation. By default,
    // uses `renderToInputAndLists`, above.
    render: renderToInputAndLists,
  ...
  // Make Flapjack available as a jQuery node function. e.g.
  // `$('select#cheeses').flapjack();
  jQuery.fn.flapjack = function() {
    $(this).each(function() {
      new Flapjack($(this));
    });
  };
